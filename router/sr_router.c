        ...
        char* interface/* lent */)
{
  ...
  sr_ethernet_hdr_t* sr_eth_hdr = get_eth_hdr(packet);
  ...
  ip_hdr->ip_sum = 0;
  uint16_t actual_checksum = cksum(ip_hdr, sizeof(sr_ip_hdr_t));
  ...
  /* If the IP Packet is not for interfaces of the router. */
    ...
    if (table_entry) {
        ...
        if (arp_entry) {
          ...
          // memcpy(sr_eth_hdr->ether_shost, rt_out_interface->addr, sizeof(uint8_t)*ETHER_ADDR_LEN);
          // memcpy(sr_eth_hdr->ether_dhost, arp_entry->mac, sizeof(unsigned char)*ETHER_ADDR_LEN);
